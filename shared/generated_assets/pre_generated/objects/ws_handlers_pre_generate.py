...
def _save_generated_question(pdf_name, data_entry):
    """Appends the new generated question object to the JSON file."""
    # Usa la funzione helper creata prima per ottenere il path
    file_path = _get_json_path(pdf_name, PRE_GEN_QUESTIONS_DIR)
    
    if not file_path:
        logger.error("Save failed: PRE_GEN_QUESTIONS_DIR not configured.")
        return
    ...
    if file_path.exists():
        try:
            ...
            logger.error(f"Failed to read existing pre-gen file: {e}")

    # 2. Calcola ID incrementale
    next_id = 1
    if current_data:
        # Trova il max id corrente
        max_id = max((item.get("question_id", 0) for item in current_data), default=0)
        next_id = max_id + 1

    data_entry["question_id"] = next_id
    ...
    try:
        ...
        logger.info(f"Saved pre-generated question {next_id} for {pdf_name}")
    except Exception as e:
        logger.error(f"Failed to write pre-gen file: {e}")
...
def handle_ask(data):
    ...
    # --- NUOVO PARAMETRO ---
    pre_gen_enabled = data.get("pre_generate_question", False)
    # -----------------------
    ...
    try:
        ...
        # --- STRUTTURE PER CATTURA DATI ---
        captured_text_chunks = []
        captured_audio_sentences = []
        captured_object_data = {} # { "object":..., "text":..., "speech":... }
        audio_worker_done = Event() # Evento per sincronizzazione
        # ----------------------------------
        ...
        # Modifica Audio Worker per catturare i file generati
        def audio_worker(sid_local, ctx, out_list, done_event):
            ...
            try:
                while True:
                    ...
                    try:
                        ...
                        # Appendiamo None o stringa vuota per mantenere l'indice allineato, se necessario
                        out_list.append("") 

                    socketio.sleep(0)
                    audio_queue.task_done()
            finally:
                # Segnala che il worker ha finito
                done_event.send()


        socketio.start_background_task(audio_worker, sid, worker_ctx, captured_audio_sentences, audio_worker_done)
        ...
        try:
            ...
            for text_content, message_type in stream_text_answer_by_sentence(transcription, language, pdf_name, sid):
                ...
                if message_type == "speech":
                    ...
                    captured_text_chunks.append(text_content) # CATTURA
            ...
            # Variabili per cattura immagine summary
            captured_summary_img_id = ""
                ...
                if summary_img_blueprint_text:
                    ...
                    try:
                        ...
                        if txt2img_response.status_code == 200:
                            ...
                            captured_summary_img_id = img_id # CATTURA
                            emit("summary_image", {
                                "image_id": img_id,
                        ...
                        logger.error("txt2img_failed", error=str(e))

                if (object_gen and obj_blueprint_text):
                    if pre_gen_enabled:
                        # Se dobbiamo salvare, eseguiamo in modo SINCRONO (o aspettiamo)
                        # per avere i dati pronti per il JSON
                        logger.info("Executing object generation synchronously for pre-gen save...")
                        run_object_generation(
                            sid, obj_blueprint_text, obj_presentation_text, 
                            language, headers, worker_ctx, 
                            output_container=captured_object_data # Passiamo il container
                        )
                    else:
                        # Comportamento standard background
                        socketio.start_background_task(
                            run_object_generation, sid, obj_blueprint_text, obj_presentation_text, 
                            language, headers, worker_ctx, 
                            output_container=None
                        )

            # Final Logging & Emitter
            answer_text = " ".join(full_answer)
            emit("text_done", {"status": "completed", "request_id": request_id})

            # --- LOGICA DI SALVATAGGIO JSON (PRE_GEN) ---
            if pre_gen_enabled:
                logger.info("Waiting for audio worker to finish before saving...")
                # Chiudiamo la coda
                audio_queue.put(None)
                # Aspettiamo che l'audio worker finisca (con timeout di sicurezza)
                try:
                    audio_worker_done.wait(timeout=30)
                except Exception as e:
                    logger.error("Timeout waiting for audio worker", error=str(e))

                # Costruzione oggetto JSON
                new_entry = {
                    "question_id": -1, # Verrà sovrascritto dalla helper
                    "text_question": transcription,
                    "text_response": answer_text,
                    "text_chunks": captured_text_chunks,
                    "summary": {
                        "title": summary_title,
                        "body": summary_bullets
                    },
                    "audio_sentences": captured_audio_sentences,
                    "summary_image": {
                        "image_id": captured_summary_img_id,
                        "caption": summary_img_caption if captured_summary_img_id else ""
                    },
                    "object": captured_object_data # Conterrà 'object', 'text', 'speech' se generati
                }
                
                # Salvataggio su file
                _save_generated_question(pdf_name, new_entry)
            
            else:
                # Se NON stiamo salvando, dobbiamo comunque chiudere la coda audio 
                # (lo faceva il blocco finally originale, lo rimettiamo qui o sotto)
                audio_queue.put(None)

        except Exception as e:
            logger.exception("error_in_ask_process", error=str(e))
            emit("error", {"message": str(e)})
            audio_queue.put(None) # Assicuriamoci di sbloccare il worker
...
def run_object_generation(sid, obj_blueprint_text, obj_presentation_text, language, headers, ctx, output_container=None):
    structlog.contextvars.bind_contextvars(**ctx)
    try:
        socketio.sleep(0)
        obj_presentation_audio = synthesize_wav(obj_presentation_text, language, context_data=ctx)

        # CATTURA AUDIO PER IL JSON
        if output_container is not None:
            output_container["speech"] = obj_presentation_audio
        ...
        try:
            ...
            if sid: socketio.emit("objects_done", {"status": "completed", "request_id": ctx.get("request_id")}, to=sid)
            return
        ...
        if txt2img_response.status_code == 200:
            ...
            if img2obj_response.status_code == 200:
                ...
                # CATTURA ID OGGETTO PER IL JSON
                if output_container is not None:
                    output_container["object"] = obj_filename
                    output_container["text"] = obj_presentation_text
