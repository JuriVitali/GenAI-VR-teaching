...
def get_text_answer(question: str, session_id = "1") -> str: #TODO:Togliere il valore di default e mettere session_id:str
    """
    Return the text response to the question using short-term memory.
    Args:
        question: The user's input.
        session_id: A unique ID for the user session (e.g., UUID or Username).
    """
    logger.info(f"Starting text answer generation for session {session_id}")
    start_text_answer_generation = time.time()

    # Define the config with the thread_id for memory
    config = {"configurable": {"thread_id": session_id}}

    # The input to the graph is the new message
    # Note: If this is a new session, you might want to prepend the SystemPrompt here
    input_messages = [HumanMessage(content=question)]

    # If you want the System Prompt to be persistent, you can check if history is empty
    # For now, let's assume the LLM knows its role or we prepend it:
    # input_messages = [SystemMessage(content=tutor_config["prompt"]), HumanMessage(content=question)]
    
    # Invoke the agent
    # The output contains the final state. We want the last message content.
