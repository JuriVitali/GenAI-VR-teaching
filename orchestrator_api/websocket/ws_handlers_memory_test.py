    ...
    try:
        ...
        def audio_worker(sid):
            while True:
                ...
                try:
                    # --- THE MAGIC FIX ---
                    # tpool.execute runs this function in a REAL OS thread, 
                    # preventing it from blocking the main Eventlet loop.
        ...
        try:
            ...
            # 3. Stream Finished: Emit the Whole Summary Event
            ...
            if object_gen:
                socketio.start_background_task(
                    run_object_generation, sid, transcription, answer_text, object_gen, max_objects, language
                )
...
def run_object_generation(sid, transcription, answer_text, object_gen, max_objects, language):
    ...
    scene_items = extract_scene_objects(transcription, answer_text)
    num_generated_obj = 0
    ...
    try:
        for item in scene_items:
            ...
            if (num_generated_obj < max_objects):
                ...
                # Extract specific fields from the Pydantic object
                text_prompt = item.prompt
                obj_presentation_text = item.speech

                socketio.sleep(0)

                obj_presentation_audio = synthesize_wav(obj_presentation_text, language)
